  ...
  const links = [
    {
      ...
      icon: <FaUserCircle className="text-[#009CA6]  h-5 w-5 flex-shrink-0" />,
    },
    {
      label: "Tickets",
      href: user?.role === "admin" ? "/admin/tickets" : "/user/tickets",
      icon: (
        <TbHeartHandshake className="text-[#009CA6] h-5 w-5 flex-shrink-0" />
      ),
    },
  ];
    ...
    <div className="border-b-2 py-2">
      <div className="flex flex-col items-center">
        <Image
          ...
          height={100}
        />
        <h4 className="font-bold mt-3 text-[#009CA6] ">
          {user?.fname || "John Doe"}
        </h4>
        <p>{user?.role}</p>
        <div className="flex gap-10 my-3 text-[#009CA6]">
          {links.map((link, idx) => (
